...
def compute_fixed_points(knockout_model, variables, continuous):
    ...
    if macaulay_executable is None:
        return make_response(
            error_report(
                "Macaulay2 was not found on the server; we cannot do as you ask."
            )
        )
    ...
    with tempfile.TemporaryDirectory() as tmp_dir_name:
        ...
        non_continuous_vars = [
            variable for variable in variables if not continuous[variable]
        ]
        if len(non_continuous_vars) > 0:
            continuity_params = ["-c", "-comit"] + [
                variable for variable in variables if not continuous[variable]
            ]
        else:
            continuity_params = ["-c"]

        convert_to_poly_process = subprocess.run(
            [
                get_resource_path("scp_converter.py"),
                "-n",
                "-i",
                tmp_dir_name + "/model.txt",
                "-o",
                tmp_dir_name + "/model-polys.txt",
            ]
            + continuity_params,
            capture_output=True,
        )
        ...
        if find_steady_states_process.returncode != 0:
            ...
            # otherwise.
            if "-infinity (of class InfiniteNumber)" in str(
                find_steady_states_process.stderr
            ):
                return make_response(
                    message(
